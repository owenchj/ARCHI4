...
int _main(int argc, char *argv[])
{
    ...
    int     nb_procs            = 1;                   // number of processor
    char    proc_name[4][20]    = {"proc_0", "proc_1", "proc_2", "proc_3"};
    ...
    if (argc > 1)
    {
        for( int n=1 ; n<argc ; n=n+2 )
        {
            ...
            else if( (strcmp(argv[n],"-NBPROCS") == 0) && (n+1<argc) )
            {
            ...
            else
            {
                ...
                std::cout << "   -NBPROCS number_of_processor" << std::endl;
    ...
    VciSignals<vci_param>   *signal_vci_init_procs = alloc_elems<VciSignals<vci_param> > ("signal_vci_init_procs", nb_procs);
    ...
    // PROC
    VciXcacheWrapper<vci_param, Mips32ElIss >* procs[nb_procs];

    for(int i = 0 ; i < nb_procs; i++)
    {
        procs[i] = new VciXcacheWrapper<vci_param,Mips32ElIss>(proc_name[i], i, maptab, i,
    ...
    tty = new VciMultiTty<vci_param>("tty", IntTab(TGTID_TTY), maptab, names);
    ...
    timer = new VciTimer<vci_param>("timer", IntTab(TGTID_TIM), maptab, nb_procs);
    ...
    for(int i = 0 ; i < nb_procs; i++)
    {
        ...
        procs[i]->p_vci                     (signal_vci_init_procs[i]);
        procs[i]->p_irq[0]                  (signal_irq_procs[i]);
        procs[i]->p_irq[1]                  (signal_false);
        procs[i]->p_irq[2]                  (signal_false);
        procs[i]->p_irq[3]                  (signal_false);
        procs[i]->p_irq[4]                  (signal_false);
        procs[i]->p_irq[5]                  (signal_false);
    }
    ...
    multi_icu->p_clk                      (signal_clk);
    multi_icu->p_resetn                   (signal_resetn);
    multi_icu->p_vci                      (signal_vci_tgt_icu);

    // multi_icu->p_irq_out[0]               (signal_irq_procs[0]);
    // multi_icu->p_irq_in[0]                (signal_irq_tims[0]);
    // multi_icu->p_irq_in[1]                (signal_irq_ttys[0]);
    // multi_icu->p_irq_in[2]                (signal_irq_ioc);
    // multi_icu->p_irq_in[3]                (signal_irq_dmas[0]);
    ...
    multi_dma->p_clk                      (signal_clk);
    multi_dma->p_resetn                   (signal_resetn);
    multi_dma->p_vci_initiator            (signal_vci_init_dma);
    multi_dma->p_vci_target               (signal_vci_tgt_dma);

    for(int i = 0 ; i < nb_procs; i++)
    {
        multi_dma->p_irq[i]               (signal_irq_dmas[i]);
    }
    ...
    for(int i = 0 ; i < nb_procs; i++)
    {
        bus->p_to_initiator[i] (signal_vci_init_procs[i]);
    }

    bus->p_to_initiator[nb_procs+1]  (signal_vci_init_dma);
    bus->p_to_initiator[nb_procs]  (signal_vci_init_ioc);
    ...
    for ( int n=1 ; n<ncycles ; n++ )
    {
        if( debug && (n > from_cycle) )
        {
            ...
            for(int i = 0 ; i < nb_procs; i++)
            {
                ...
                if( signal_irq_procs[i].read() ) std::cout << "IRQ_PROC" << std::endl;
                if( signal_irq_tims[i].read() )  std::cout << "IRQ_TIM"  << std::endl;
                if( signal_irq_ttys[i].read() )  std::cout << "IRQ_TTY"  << std::endl;
                if( signal_irq_dmas[i].read() )  std::cout << "IRQ_DMA"  << std::endl;
            }
